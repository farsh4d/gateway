...
در آخرین تغییرات این پکیج تنظیمات مربوط به درگاه ها دیگر از فایل `config/gateway.php` فراخوانی نمیشود و بجای آن از روی دیتابیس فراخوانی صورت میگیرد.
همچنین این امکان فراهم آمده که برای یک درگاه بیش از یک Config ایجاد کرد.
برای تنظیم Config پیشفرض کافیست در فایل `config/gateway.php` در بخش `default_config_id` مقدار ID مربوط به تنظیمات موردنظر را در مقابل نام هر درگاه وارد کنید.
